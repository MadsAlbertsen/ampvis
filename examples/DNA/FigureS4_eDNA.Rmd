...
## Figure S4C: PCA colored by PMA treatment
```{r edna_PCA}
...
```{r save_S4D, eval=FALSE}
ggsave("plots/S4D.eps", width = 60, height = 55, units = "mm")
```

## Figure S4E: Variance compared to time-series data
```{r edna_time}
...
```{r save_S4E, eval=FALSE}
ggsave("plots/S4E.eps", width = 90, height = 55, units = "mm")
```

## Figure S4F: Using clustering to estimate classification resolution
```{r beta_time}
...
```{r save_S4F, eval=FALSE}
ggsave("plots/S4F.eps", width = 60, height = 55, units = "mm")
```

## Figure S4B: Impact of eDNA removal on Alpha-diversity
